...
public class TableFilter extends RowFilter<TableModel, Integer> {
    ...
    public void initFilterItems() {
     ...
     /**
     *  this filters the table by the underlying column value corresponding 
     * to the item double clicked on 
     */
    public void filterByDoubleClick() {
        int rowIndex = table.getSelectedRow(); // this returns the rowIndex i
        if (rowIndex != -1) {
            Object selectedField = table.getValueAt(rowIndex, underlyingColumnIndex);

           addFilterItem(underlyingColumnIndex, selectedField);
            applyFilter();
        }
    }

    /**
     * This clears the filters for the column that is double
     * clicked on.
     */
    public void clearFilterByDoubleClick(int columnIndex) {
        
        // clear column filter
        int symbolColumnIndex = getSymbolColumnIndex();
        if (columnIndex == symbolColumnIndex) {
            int underlyingColumnIndex = getUnderlyingColumnIndex();
            clearColFilter(underlyingColumnIndex);
            removeColorHeader(symbolColumnIndex);
        } 
        else {
            clearColFilter(columnIndex);
        }
        applyFilter();
    }
    
    //filters by the symbol parameter
    public void filterBySymbol(String symbol)
    {// apply filter for the symbol
        int colIndexUL = getUnderlyingColumnIndex();   // underlying column i
        addFilterItem(colIndexUL, symbol);
        removeColorHeader(colIndexUL);
        applyFilter();
        int colIndexSymbol = getSymbolColumnIndex();  // symbol column i
        addColorHeader(colIndexSymbol);
    }
    
    //filters column by date
    public void filterByDate(Date startDateRange, Date endDateRange){
        // execute filter
        int dateColumnIndex = getDateColumnIndex();
        clearColFilter(dateColumnIndex);
        addDateRange(startDateRange, endDateRange);
        applyFilter();
    }
    ...
    public void removeFilter(String filterName){
       ...
       columnIndex = getDateColumnIndex();
        }
        else if(filterName.toLowerCase().contains("symbol")){
            int underlyingColumnIndex = getUnderlyingColumnIndex();
           columnIndex = underlyingColumnIndex;
            int symbolColumnIndex = getSymbolColumnIndex();
            removeColorHeader(symbolColumnIndex);
        }
        clearColFilter(columnIndex);
    ...
    //removes the filters from all columns
    ...
    public void clearColFilter(int columnIndex) {
        filterItems.get(columnIndex).clear(); // remove the filters for the column
    ...
    public void setDateColumnIndex() {
        ...
        for (col = 0; col < table.getColumnCount(); col++) {
            ...
            if (colName.toLowerCase().contains("lot_time") || colName.toLowerCase().contains("trade_time")) {
                 dateColumnIndex = col;
                break;
            }
        }
    ...
    public void setUnderlyingColumnIndex() {
        ...
        for (col = 0; col < table.getColumnCount(); col++) {
            String colName = table.getColumnName(col);
            if (colName.toLowerCase().contains("underlying")) {
